...
bool SUP_DataParser::processLine(QString line)
{
    ...
    if (state == State::LookingForData)
    {
        ...
        else if (lineIsDictBegin(line))
        {
            state = State::ParsingEntriesOfDict;
            return true;
        }
        else return true;
    }
    ...
    else if (state == State::ParsingEntriesOfDict)
    {
        return processDictEntryLine(line);
    }
...
bool SUP_DataParser::lineIsDictBegin(const QString &line)
{
    return line == DictBegin;
}

bool SUP_DataParser::lineIsDictEnd(const QString &line)
{
    return line == DictEnd;
}
